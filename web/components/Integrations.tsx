"use client"

import { motion } from "framer-motion"

const INTEGRATIONS = [
  { name: "Stripe", className: "h-8 md:h-9", logo: (props: any) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 60" fillRule="evenodd" {...props}>
      <path fill="#6772e5" d="M101.547 30.94c0-5.885-2.85-10.53-8.3-10.53-5.47 0-8.782 4.644-8.782 10.483 0 6.92 3.908 10.414 9.517 10.414 2.736 0 4.805-.62 6.368-1.494v-4.598c-1.563.782-3.356 1.264-5.632 1.264-2.23 0-4.207-.782-4.46-3.494h11.24c0-.3.046-1.494.046-2.046zM90.2 28.757c0-2.598 1.586-3.678 3.035-3.678 1.402 0 2.897 1.08 2.897 3.678zm-14.597-8.345c-2.253 0-3.7 1.057-4.506 1.793l-.3-1.425H65.73v26.805l5.747-1.218.023-6.506c.828.598 2.046 1.448 4.07 1.448 4.115 0 7.862-3.3 7.862-10.598-.023-6.667-3.816-10.3-7.84-10.3zm-1.38 15.84c-1.356 0-2.16-.483-2.713-1.08l-.023-8.53c.598-.667 1.425-1.126 2.736-1.126 2.092 0 3.54 2.345 3.54 5.356 0 3.08-1.425 5.38-3.54 5.38zm-16.4-17.196l5.77-1.24V13.15l-5.77 1.218zm0 1.747h5.77v20.115h-5.77zm-6.185 1.7l-.368-1.7h-4.966V40.92h5.747V27.286c1.356-1.77 3.655-1.448 4.368-1.195v-5.287c-.736-.276-3.425-.782-4.782 1.7zm-11.494-6.7L34.535 17l-.023 18.414c0 3.402 2.552 5.908 5.954 5.908 1.885 0 3.264-.345 4.023-.76v-4.667c-.736.3-4.368 1.356-4.368-2.046V25.7h4.368v-4.897h-4.37zm-15.54 10.828c0-.897.736-1.24 1.954-1.24a12.85 12.85 0 0 1 5.7 1.47V21.47c-1.908-.76-3.793-1.057-5.7-1.057-4.667 0-7.77 2.437-7.77 6.506 0 6.345 8.736 5.333 8.736 8.07 0 1.057-.92 1.402-2.207 1.402-1.908 0-4.345-.782-6.276-1.84v5.47c2.138.92 4.3 1.3 6.276 1.3 4.782 0 8.07-2.368 8.07-6.483-.023-6.85-8.782-5.632-8.782-8.207z"/>
    </svg>
  )},
  { name: "Google Analytics", className: "h-6 md:h-7", logo: (props: any) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="26 -29 120 60" {...props}>
      <defs>
        <linearGradient id="ga-gradient" gradientUnits="userSpaceOnUse" x1="173.867" y1="-72.65" x2="216.749" y2="-72.65">
          <stop offset="0" stopColor="#e96f0b"/>
          <stop offset="1" stopColor="#f37901"/>
        </linearGradient>
      </defs>
      <g className="fill-[#757575] dark:fill-white">
        <path d="M74.093 7.228l2.388 6.27h-4.627l2.24-6.27zm-.896-2.1l-5.224 13.73h1.94l1.343-3.73h5.82l1.343 3.73h1.94l-5.224-13.73h-1.94zm28.2 13.742h1.8V5.138h-1.8v13.73zm-18.35-8.07c.597-.896 1.8-1.642 2.985-1.642 2.388 0 3.582 1.642 3.582 4.03v5.82h-1.8v-5.522c0-1.94-1.045-2.687-2.388-2.687-1.493 0-2.537 1.493-2.537 2.836v5.224h-1.8V9.32h1.8l.15 1.493zm8.06 5.234c0-2.1 1.94-3.284 4.18-3.284 1.343 0 2.24.3 2.537.597v-.3c0-1.493-1.194-2.24-2.388-2.24-1.045 0-1.94.448-2.24 1.343l-1.642-.746c.3-.896 1.493-2.24 3.88-2.24 2.24 0 4.18 1.343 4.18 4.03v5.672h-1.642v-1.343h-.15c-.448.746-1.493 1.642-3.134 1.642-1.94 0-3.582-1.194-3.582-3.134m6.716-1.194s-.746-.597-2.24-.597c-1.8 0-2.537 1.045-2.537 1.8 0 1.045 1.045 1.493 1.94 1.493 1.343 0 2.836-1.194 2.836-2.687"/><path d="M105.735 23.05l2.1-4.925-3.73-8.657h1.8l2.836 6.567 2.836-6.567h1.8l-5.82 13.582h-1.8zm26.27-10.45c-.448-1.194-1.493-1.94-2.537-1.94-1.493 0-2.836 1.343-2.836 3.284s1.343 3.284 2.836 3.284c1.045 0 2.1-.746 2.537-1.8l1.493.896c-.746 1.642-2.24 2.687-4.03 2.687-2.537 0-4.627-2.24-4.627-5.075 0-2.985 2.1-5.075 4.627-5.075 1.8 0 3.284 1.045 4.03 2.687l-1.493 1.045z" fillRule="evenodd"/><path d="M138.422 19.168c2.388 0 3.582-1.343 3.582-2.985 0-3.582-5.224-2.24-5.224-4.328 0-.746.597-1.194 1.642-1.194s2.1.448 2.537 1.194l1.045-1.045c-.597-.746-2.24-1.8-3.73-1.8-2.24 0-3.433 1.343-3.433 2.985 0 3.433 5.373 2.24 5.373 4.03 0 .896-.597 1.493-1.8 1.493s-1.8-.746-2.388-1.493L134.7 17.08c.896.896 2.24 2.1 3.73 2.1zm-17.015-.298h1.8V9.318h-1.8v9.552z"/><path d="M122.153 4.7c.746 0 1.194.597 1.194 1.194 0 .746-.597 1.194-1.194 1.194s-1.194-.597-1.194-1.194.597-1.194 1.194-1.194zm-2.537 12.687l.3 1.343h-1.8c-1.642 0-2.388-1.194-2.388-2.985V11.1h-1.8V9.3h1.8V6.482h1.8v2.836h2.1v1.8h-2.1v4.925c0 1.343 1.045 1.343 2.1 1.343z" fillRule="evenodd"/><path d="M77.048-10.042v2.24h5.224c-.15 1.194-.597 2.1-1.194 2.687-.746.746-1.94 1.642-4.03 1.642-3.134 0-5.672-2.537-5.672-5.82 0-3.134 2.537-5.82 5.672-5.82 1.8 0 2.985.746 3.88 1.493l1.493-1.493c-1.343-1.194-2.985-2.24-5.373-2.24-4.328 0-8.06 3.582-8.06 7.9s3.73 7.9 8.06 7.9c2.388 0 4.18-.746 5.522-2.24 1.493-1.493 1.94-3.433 1.94-5.075 0-.448 0-1.045-.15-1.343h-7.313zm13.282-1.8c-2.836 0-5.075 2.1-5.075 5.075s2.24 5.075 5.075 5.075a5.03 5.03 0 0 0 5.075-5.075c0-2.985-2.24-5.075-5.075-5.075zm0 8.2c-1.493 0-2.836-1.343-2.836-3.134S88.837-9.9 90.33-9.9s2.837 1.2 2.837 3.13-1.343 3.134-2.836 3.134zm24.47-7.156c-.597-.597-1.493-1.194-2.836-1.194-2.537 0-4.776 2.24-4.776 5.075s2.24 5.075 4.776 5.075c1.194 0 2.24-.597 2.687-1.194h.15v.746c0 1.94-1.045 2.985-2.687 2.985-1.343 0-2.24-1.045-2.537-1.8l-1.94.746c.597 1.343 2.1 2.985 4.478 2.985 2.687 0 4.925-1.493 4.925-5.373V-12h-2.1v1.194zm-2.537 7.164c-1.493 0-2.687-1.343-2.687-3.134s1.194-3.134 2.687-3.134 2.687 1.343 2.687 3.134-1.194 3.134-2.687 3.134zm-10.897-8.2c-2.836 0-5.075 2.1-5.075 5.075s2.24 5.075 5.075 5.075a5.03 5.03 0 0 0 5.075-5.075c.15-2.985-2.24-5.075-5.075-5.075zm0 8.2c-1.493 0-2.836-1.343-2.836-3.134s1.343-3.134 2.836-3.134 2.836 1.194 2.836 3.134-1.343 3.134-2.836 3.134zM118.7-17.206h2.24v15.522h-2.24v-15.522zm8.358 13.582c-1.194 0-1.94-.597-2.537-1.493l6.866-2.836-.3-.597c-.448-1.194-1.8-3.284-4.328-3.284-2.687 0-4.776 2.1-4.776 5.075a5.03 5.03 0 0 0 5.075 5.075c2.388 0 3.73-1.493 4.328-2.24l-1.8-1.194c-.597.896-1.343 1.493-2.537 1.493zm-.15-6.27c.896 0 1.642.448 1.94 1.194l-4.627 1.94c0-2.24 1.493-3.134 2.687-3.134z"/>
      </g>
      <g transform="matrix(.198617 0 0 .198616 29.128442 -18.08775)">
        <path d="M130 29v132c0 14.77 10.2 23 21 23 10 0 21-7 21-23V30c0-13.54-10-22-21-22s-21 9.33-21 21z" fill="#f9ab00"/>
        <g fill="#e37400">
          <path d="M75 96v65c0 14.77 10.2 23 21 23 10 0 21-7 21-23V97c0-13.54-10-22-21-22s-21 9.33-21 21z"/>
          <circle r="21" cy="163" cx="41"/>
        </g>
      </g>
    </svg>
  )},
  { name: "Meta", className: "h-5 md:h-6", logo: (props: any) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 103 24" fillRule="evenodd" {...props}>
      <defs>
        <linearGradient id="meta-fill-0" x1="75.904%" x2="33.275%" y1="89.153%" y2="23.044%">
          <stop offset=".06%" stopColor="#0867DF"/>
          <stop offset="45.39%" stopColor="#0668E1"/>
          <stop offset="100%" stopColor="#0064E0"/>
        </linearGradient>
        <linearGradient id="meta-fill-1" x1="21.67%" x2="97.068%" y1="75.837%" y2="24.022%">
          <stop offset="13.23%" stopColor="#0064DF"/>
          <stop offset="99.88%" stopColor="#0064E0"/>
        </linearGradient>
        <linearGradient id="meta-fill-2" x1="38.247%" x2="60.91%" y1="89.127%" y2="16.131%">
          <stop offset="1.47%" stopColor="#0072EC"/>
          <stop offset="68.81%" stopColor="#0064DF"/>
        </linearGradient>
        <linearGradient id="meta-fill-3" x1="47.027%" x2="52.153%" y1="90.19%" y2="15.745%">
          <stop offset="7.31%" stopColor="#007CF6"/>
          <stop offset="99.43%" stopColor="#0072EC"/>
        </linearGradient>
        <linearGradient id="meta-fill-4" x1="52.155%" x2="47.591%" y1="58.289%" y2="37.023%">
          <stop offset="7.31%" stopColor="#007FF9"/>
          <stop offset="100%" stopColor="#007CF6"/>
        </linearGradient>
        <linearGradient id="meta-fill-5" x1="37.689%" x2="61.961%" y1="12.556%" y2="63.605%">
          <stop offset="7.31%" stopColor="#007FF9"/>
          <stop offset="100%" stopColor="#0082FB"/>
        </linearGradient>
        <linearGradient id="meta-fill-6" x1="42.496%" x2="59.964%" y1="56.072%" y2="27.099%">
          <stop offset="0%" stopColor="#007FF8"/>
          <stop offset="100%" stopColor="#0082FB"/>
        </linearGradient>
        <linearGradient id="meta-fill-7" x1="43.753%" x2="57.613%" y1="6.235%" y2="98.514%">
          <stop offset="0%" stopColor="#0082FB"/>
          <stop offset="99.95%" stopColor="#0081FA"/>
        </linearGradient>
        <linearGradient id="meta-fill-8" x1="60.07%" x2="39.865%" y1="4.661%" y2="69.077%">
          <stop offset="6.19%" stopColor="#0081FA"/>
          <stop offset="100%" stopColor="#0080F9"/>
        </linearGradient>
        <linearGradient id="meta-fill-9" x1="30.254%" x2="61.097%" y1="59.32%" y2="33.244%">
          <stop offset="0%" stopColor="#027AF3"/>
          <stop offset="100%" stopColor="#0080F9"/>
        </linearGradient>
        <linearGradient id="meta-fill-10" x1="20.433%" x2="82.112%" y1="50.001%" y2="50.001%">
          <stop offset="0%" stopColor="#0377EF"/>
          <stop offset="99.94%" stopColor="#0279F1"/>
        </linearGradient>
        <linearGradient id="meta-fill-11" x1="40.289%" x2="72.427%" y1="35.298%" y2="57.811%">
          <stop offset=".19%" stopColor="#0471E9"/>
          <stop offset="100%" stopColor="#0377EF"/>
        </linearGradient>
        <linearGradient id="meta-fill-12" x1="32.228%" x2="68.028%" y1="19.719%" y2="84.908%">
          <stop offset="27.65%" stopColor="#0867DF"/>
          <stop offset="100%" stopColor="#0471E9"/>
        </linearGradient>
      </defs>
      <g className="fill-black dark:fill-white">
        <path d="M38.337 2.641h3.752l6.38 11.57 6.38-11.57h3.671v19.011H55.46V7.082l-5.594 10.09h-2.872l-5.595-10.09v14.57h-3.06V2.642zm30.058 19.353c-1.418 0-2.664-.314-3.739-.944a6.594 6.594 0 01-2.513-2.614c-.6-1.113-.9-2.39-.9-3.829 0-1.457.293-2.748.88-3.87.587-1.123 1.402-2 2.445-2.635 1.043-.633 2.242-.95 3.596-.95 1.346 0 2.504.319 3.475.957.97.638 1.718 1.532 2.242 2.682.523 1.15.785 2.499.785 4.047v.842h-10.39c.19 1.159.658 2.07 1.403 2.736.745.665 1.686.998 2.824.998.912 0 1.698-.136 2.357-.407a5.997 5.997 0 001.856-1.236l1.625 1.996c-1.616 1.485-3.599 2.227-5.946 2.227zm2.235-11.278c-.642-.656-1.481-.984-2.52-.984-1.011 0-1.858.335-2.54 1.005-.681.67-1.113 1.57-1.293 2.702h7.45c-.09-1.159-.456-2.066-1.097-2.723zm7.748-.712h-2.817V7.492h2.817V3.337h2.953v4.155h4.28v2.512h-4.28v6.369c0 1.059.18 1.815.542 2.267.36.453.98.68 1.855.68.389 0 .718-.016.99-.048l.426-.056c.15-.022.305-.046.467-.073v2.485a8.876 8.876 0 01-2.452.34c-3.187 0-4.781-1.748-4.781-5.242v-6.722zM101 21.654h-2.899v-1.982a5.24 5.24 0 01-1.964 1.718c-.795.403-1.698.604-2.71.604-1.245 0-2.35-.319-3.311-.957-.962-.638-1.718-1.517-2.269-2.635-.55-1.118-.826-2.396-.826-3.836 0-1.448.28-2.73.84-3.843.56-1.113 1.334-1.987 2.323-2.62.989-.634 2.124-.951 3.406-.951.967 0 1.834.188 2.601.563a5.145 5.145 0 011.91 1.596V7.49H101v14.164zm-2.953-9.206c-.316-.806-.815-1.442-1.497-1.908-.682-.466-1.47-.7-2.363-.7-1.265 0-2.272.426-3.021 1.277-.75.851-1.125 2-1.124 3.45 0 1.457.36 2.611 1.083 3.462.722.851 1.702 1.276 2.94 1.276.911 0 1.724-.235 2.438-.706a3.987 3.987 0 001.544-1.9v-4.251z"/>
      </g>
      <path d="M8.627 0h-.029l-.04 3.27.028-.001c2.146 0 3.81 1.696 7.43 7.807l.22.372.014.024 2.026-3.047-.014-.023a60.949 60.949 0 00-1.374-2.146 35.014 35.014 0 00-1.47-2.036C13.026 1.165 11.024 0 8.628 0z" fill="url(#meta-fill-0)" transform="translate(2 2)"/>
      <path d="M8.598 0C6.19.013 4.062 1.573 2.525 3.961a5.44 5.44 0 00-.013.021l2.82 1.54.015-.022c.897-1.354 2.014-2.218 3.211-2.23l.027-.001L8.627 0h-.03z" fill="url(#meta-fill-1)" transform="translate(2 2)"/>
      <path d="M2.525 3.963l-.013.02C1.502 5.56.749 7.494.342 9.58l-.005.027 3.17.75c0-.01.002-.018.004-.028.339-1.833.984-3.533 1.822-4.806l.013-.02-2.82-1.54z" fill="url(#meta-fill-2)" transform="translate(2 2)"/>
      <path d="M3.511 10.33L.342 9.582l-.005.027A18.541 18.541 0 000 13.09v.028l3.25.292v-.029a15.728 15.728 0 01.262-3.05z" fill="url(#meta-fill-3)" transform="translate(2 2)"/>
      <path d="M3.349 14.422a6.822 6.822 0 01-.099-1.017v-.028L0 13.085v.031a11.105 11.105 0 00.183 2.064l3.17-.731a5.131 5.131 0 01-.004-.027z" fill="url(#meta-fill-4)" transform="translate(2 2)"/>
      <path d="M4.09 16.112c-.354-.387-.605-.945-.736-1.659l-.005-.027-3.171.731.005.027c.24 1.262.71 2.313 1.383 3.108l.018.022 2.524-2.181a2.635 2.635 0 01-.019-.02z" fill="url(#meta-fill-5)" transform="translate(2 2)"/>
      <path d="M15.484 9.067a399.491 399.491 0 00-3.069 4.781c-2.545 4-3.426 4.897-4.843 4.897-.591 0-1.085-.211-1.464-.615a2.313 2.313 0 01-.019-.02l-2.523 2.18.018.021C4.514 21.397 5.825 22 7.448 22c2.456 0 4.222-1.16 7.361-6.662l2.21-3.913a51.6 51.6 0 00-1.535-2.358z" fill="#0082FB"/>
      <path d="M16.89 2.432a19.722 19.722 0 00-1.471 1.79c.472.604.96 1.281 1.47 2.038.6-.93 1.16-1.682 1.71-2.258l.019-.02-1.728-1.55z" fill="url(#meta-fill-6)" transform="translate(2 2)"/>
      <path d="M28.166 4.142C26.834 2.792 25.245 2 23.546 2c-1.79 0-3.297.984-4.656 2.43a6.451 6.451 0 00-.02.02L20.6 6l.019-.021c.895-.934 1.762-1.4 2.722-1.4 1.034 0 2.001.488 2.84 1.343l.019.02 1.987-1.78-.02-.02z" fill="#0082FB"/>
      <path d="M30.019 12.656c-.075-4.333-1.588-8.207-3.833-10.495l-.02-.02-1.987 1.78.02.02c1.689 1.74 2.848 4.975 2.953 8.714v.028l2.867.001v-.028z" fill="url(#meta-fill-7)" transform="translate(2 2)"/>
      <path d="M30.02 12.687l-.001-.028h-2.867v.027c.005.176.008.352.008.53 0 1.02-.152 1.843-.461 2.438l-.014.027 2.137 2.228.016-.024c.776-1.2 1.183-2.868 1.183-4.89 0-.103 0-.205-.002-.308z" fill="url(#meta-fill-8)" transform="translate(2 2)"/>
      <path d="M26.7 15.65l-.015.026c-.267.502-.649.836-1.147.982l.974 3.079a4.372 4.372 0 00.547-.229 4.45 4.45 0 001.709-1.522 4.18 4.18 0 00.055-.081l.015-.025-2.139-2.23z" fill="url(#meta-fill-9)" transform="translate(2 2)"/>
      <path d="M24.917 16.742c-.327 0-.615-.049-.897-.175l-.998 3.151c.56.192 1.16.279 1.826.279.615 0 1.179-.092 1.69-.27l-.974-3.077c-.21.063-.428.094-.647.092z" fill="url(#meta-fill-10)" transform="translate(2 2)"/>
      <path d="M22.92 15.667l-.018-.021-2.296 2.393.02.02c.797.853 1.56 1.382 2.423 1.672l.997-3.15c-.364-.156-.716-.44-1.125-.914z" fill="url(#meta-fill-11)" transform="translate(2 2)"/>
      <path d="M22.902 15.643c-.688-.802-1.54-2.139-2.88-4.3l-1.746-2.92-.014-.023-2.026 3.046.014.024 1.237 2.087c1.2 2.012 2.177 3.467 3.12 4.48l.019.02 2.295-2.392a2.945 2.945 0 01-.019-.022z" fill="url(#meta-fill-12)" transform="translate(2 2)"/>
    </svg>
  )},
  { name: "TikTok", className: "h-6 md:h-7", logo: (props: any) => (
    <svg xmlns="http://www.w3.org/2000/svg" xmlSpace="preserve" viewBox="0 0 240 120" {...props}>
      <path fill="#ff004f" d="M52.473 51.229a25.18 25.18 0 0 0 14.686 4.697V45.393c-1.036 0-2.07-.108-3.084-.322v8.29a25.2 25.2 0 0 1-14.686-4.696v21.494c0 10.753-8.722 19.47-19.48 19.47a19.4 19.4 0 0 1-10.843-3.294 19.43 19.43 0 0 0 13.927 5.858c10.759 0 19.48-8.717 19.48-19.47zm3.805-10.627a14.68 14.68 0 0 1-3.805-8.594v-1.355H49.55a14.75 14.75 0 0 0 6.728 9.95M25.87 78.084a8.87 8.87 0 0 1-1.817-5.392c0-4.918 3.989-8.905 8.91-8.905.917 0 1.829.14 2.703.417V53.436q-1.535-.21-3.083-.178v8.382a9 9 0 0 0-2.704-.417c-4.922 0-8.91 3.987-8.91 8.905a8.9 8.9 0 0 0 4.901 7.956"/>
      <path className="fill-black dark:fill-white" d="M49.389 48.664a25.2 25.2 0 0 0 14.686 4.697v-8.29a14.74 14.74 0 0 1-7.797-4.469 14.75 14.75 0 0 1-6.728-9.95h-7.677v42.07a8.91 8.91 0 0 1-8.91 8.876 8.9 8.9 0 0 1-7.093-3.514 8.9 8.9 0 0 1-4.902-7.955c0-4.918 3.989-8.906 8.91-8.906.943 0 1.852.147 2.705.417V53.26c-10.569.218-19.069 8.849-19.069 19.464a19.4 19.4 0 0 0 5.552 13.613c3.1 2.08 6.83 3.293 10.844 3.293 10.758 0 19.479-8.717 19.479-19.47z"/>
      <path fill="#00f2ea" d="M64.075 45.07V42.83a14.7 14.7 0 0 1-7.797-2.227 14.73 14.73 0 0 0 7.797 4.468M49.55 30.653q-.105-.602-.161-1.21V28.09h-10.6v42.07c-.017 4.904-4 8.875-8.91 8.875a8.9 8.9 0 0 1-4.01-.95 8.9 8.9 0 0 0 7.094 3.514c4.91 0 8.893-3.97 8.91-8.875v-42.07ZM32.583 53.259v-2.387a20 20 0 0 0-2.673-.181c-10.758 0-19.48 8.716-19.48 19.468 0 6.74 3.428 12.682 8.636 16.176a19.4 19.4 0 0 1-5.552-13.612c0-10.615 8.5-19.246 19.07-19.464"/>
      <path fill="#ff004f" d="M186.9 80.814c7.697 0 13.937-6.19 13.937-13.825s-6.24-13.826-13.938-13.826h-2.1c7.698 0 13.938 6.191 13.938 13.826s-6.24 13.825-13.938 13.825z"/>
      <path fill="#00f2ea" d="M184.609 53.163h-2.1c-7.697 0-13.939 6.191-13.939 13.826s6.242 13.825 13.94 13.825h2.099c-7.699 0-13.939-6.19-13.939-13.825s6.24-13.826 13.939-13.826"/>
      <path className="fill-black dark:fill-white" d="M78.644 46.915v7.007h8.209v26.703h8.212V54.112h6.681l2.292-7.197zm67.207 0v7.007h8.208v26.703h8.212V54.112h6.682l2.29-7.197zm-40.285 3.976c0-2.195 1.792-3.976 4.007-3.976 2.216 0 4.01 1.78 4.01 3.976s-1.793 3.977-4.01 3.977c-2.215 0-4.007-1.78-4.007-3.977m0 6.819h8.017v22.915h-8.017zm11.454-10.795v33.71h8.02v-8.711l2.482-2.275 7.827 11.175h8.59l-11.265-16.287 10.12-9.849h-9.737l-8.017 7.954V46.915Zm86.489 0v33.71h8.02v-8.711l2.481-2.275 7.83 11.175h8.592l-11.266-16.287 10.119-9.849h-9.736l-8.02 7.954V46.915ZM184.798 80.814c7.698 0 13.94-6.19 13.94-13.825s-6.242-13.826-13.94-13.826h-.19c-7.698 0-13.938 6.191-13.938 13.826s6.24 13.825 13.938 13.825zm-6.873-13.825c0-3.714 3.036-6.722 6.78-6.722 3.741 0 6.777 3.008 6.777 6.722s-3.036 6.724-6.777 6.724c-3.744 0-6.78-3.01-6.78-6.724"/>
    </svg>
  )},
]

export function Integrations() {
  return (
    <section className="border-b border-vasta-border bg-vasta-bg py-12">
      <div className="mx-auto max-w-7xl px-4 text-center">
        <p className="mb-8 text-xs font-bold uppercase tracking-widest text-vasta-muted">
          Integração nativa com suas ferramentas favoritas
        </p>
        
        <div className="flex flex-wrap items-center justify-center gap-8 md:gap-16">
          {INTEGRATIONS.map((tool) => (
            <motion.div 
              key={tool.name}
              whileHover={{ scale: 1.05, y: -2 }}
              className="group flex flex-col items-center gap-2 cursor-pointer"
            >
              <tool.logo className={`${tool.className || "h-10"} w-auto transition-transform`} />
            </motion.div>
          ))}
        </div>
      </div>
    </section>
  )
}
